...
from data_anjany import AnjanyDataGenerator
...
aggregation_plane = 'coronal'#'sagittal'
...
ag = AnchorGenerator(feature_stride=32,
                     ...
                     scale=scales)
gen = AnjanyDataGenerator(batch_size=50,
                          folder_path='/media/Data/Datasets/xVertSeg.v1/aux_data/2d_reformations/{}/train/'.format(aggregation_plane),
                          overlap_threshold=.5,
                          anchor_generator=ag)
...
with open('output/residual_ssd/cts_{}_max_train_spine/epoch_150.pkl'.format(aggregation_plane), 'rb') as f:
    w = cPickle.load(f)
model.set_weights(w)
print 'Model ready.'
...
top_k = np.argsort(-np.max(pred[img_index,:,5:], axis=-1))[:10] # k = 1
