...
#  jan.kukacka@tum.de
# ------------------------------------------------------------------------------
#  Data generator from Anjany's files
# ------------------------------------------------------------------------------
...
class AnjanyDataGenerator(object):
    '''
    ...
    def __init__(self, batch_size, folder_path, padding=0, min_voxels=500,
                 max_images=-1, use_two_classes=False, anchor_generator=None,
                 overlap_threshold=.5, match_anchors=True, min_wh_ratio=0.3):
        '''
        ...
        if not use_two_classes:
            ...
            self.classes = ('__background__', 'spine')
        ...
        for filename in os.listdir(folder_path):
            if filename.endswith('.npz'):
                self.images[filename] =  np.load(os.path.join(folder_path, filename))['im'][::-1,:]
                if 'sagittal' in folder_path:
                    self.images[filename] =  np.transpose(self.images[filename], (1,0))
                self.image_index.append(filename)
        print 'Finished.'
        ...
        stdevs = []
        sizes = []
        for im in self.image_index:
            means.append(np.mean(self.images[im]))
            stdevs.append(np.var(self.images[im]))
            sizes.append(np.size(self.images[im]))
        means = np.array(means)
        stdevs = np.array(stdevs)
        sizes = np.array(sizes)
        mean = np.sum(means * sizes) / np.sum(sizes)
        var = np.sum((sizes - 1)*stdevs) / np.sum(sizes - 1)
        std = np.sqrt(var)
        for im in self.image_index:
            self.images[im] = (self.images[im] - mean)/std
        print 'ZMUV finished.'
    ...
    def Generate(self, shuffle=True):
        print 'Warning: This generator generates no ground truth boxes.'
